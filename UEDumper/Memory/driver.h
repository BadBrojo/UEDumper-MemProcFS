...
inline void loadData(std::string& processName, uint64_t& baseAddress, int& processID)
{
    ...
    baseAddress = getBaseAddress(wProcessName.c_str(), processID);  // Now passing const wchar_t*
}
...
uint64_t getBaseAddress(const wchar_t* processName, int& pid)
{
    ...
    ULONG64 baseAddress = VMMDLL_ProcessGetModuleBaseW(hVMM, pid, const_cast<LPWSTR>(processName));
    if (baseAddress == 0)
    {
 ...
 */
void attachToProcess(const int& processID)
{
